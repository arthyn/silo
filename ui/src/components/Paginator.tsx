    ...
    <>
      {showNextPrev && (
        <div className="-mt-px w-0 flex-1 flex">
          {!prevUrl && (
            <span className='border-t-2 border-transparent pt-2 pl-1 inline-flex items-center text-sm font-medium text-gray-300'>
              <ArrowNarrowLeftIcon className="mr-3 h-5 w-5" />
              Previous
            </span>
          )}
          {prevUrl && <Link to={prevUrl} className="border-t-2 border-transparent pt-2 pr-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
      ...
      {pageSet.map((page, index) => {
        if (page.ellipsis)
            return <span key={index} className="border-transparent text-gray-500 border-t-2 pt-2 px-4 inline-flex items-center text-sm font-medium"><DotsHorizontalIcon className='w-3 h-3' /></span>
        ...
        return (
          <Link 
            ...
            className={classNames(
              ...
              page.active && 'border-indigo-500 text-indigo-600',
              !page.active && 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
            )}
      ...
      {showNextPrev && (
        <div className="-mt-px w-0 flex-1 flex justify-end">
          ...
          {nextUrl && <Link to={nextUrl} className="border-t-2 border-transparent pt-2 pl-1 inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300">
            Next
